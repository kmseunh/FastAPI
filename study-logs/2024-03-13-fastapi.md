...
# Pydantic을 사용하여 데이터 모델 정의
# Request Body에 보낼 데이터의 구조
...
2. 모델 사용하기
함수 안에서 모델 객체의 모든 어트리뷰트에 직접 접근 가능하다.
...
# # Query 함수를 사용하여 q 매개변수를 정의, 기본값은 None이고, 최대 길이는 50
@app.get("/items/")
...
# 매개변수의 기본값은 "fixedquery", 최소 길이는 3으로 제한
@app.get("/items/")
...
> 매개변수를 사용하는 클라이언트가 있기 때문에 한동안은 남겨둬야 하지만, 사용되지 않는다(`deprecated`)고 확실하게 문서에서 보여주고 싶다면 `deprecated=True` 매개변수를 `Query`로 전달합니다:
