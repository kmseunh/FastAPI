...
`의존성 주입`은 공용된 로직을 가졌을 경우, 데이터베이스 연결을 공유하는 경우, 보안 및 인증, 역할 요구 사항 등을 강제하는 경우 등에 코드 반복을 최소화한다.
...
# Depends 함수를 사용하여 common_parameters 함수를 의존성으로 지정
@app.get("/items/")
...
# Annotated를 사용하여 CommonsDep 정의
CommonsDep = Annotated[dict, Depends(common_parameters)]


# common_parameters 함수의 반환값을 사용하는 의존성
@app.get("/items/")
...
# CommonQueryParams 클래스의 인스턴스로서,공통 쿼리 매개변수를 사용하는 의존성
@app.get("/items/")
async def read_items(commons: CommonQueryParams = Depends(CommonQueryParams)): 
    """
    Parameters:
        commons (CommonQueryParams): CommonQueryParams 클래스의 인스턴스로서,
        공통 쿼리 매개변수를 사용하는 의존성
    """
    response = {}
    if commons.q:
